...
import { recalcLine, updateSaleItemSummary } from "./sale-item-summary";
import { updateTotalSaleDisplay } from "./sale-summary";
import { fetchProductSuggestions } from "./sales-service";
  ...
  const numericInputs = tr.querySelectorAll<HTMLInputElement>(
    ...
    numericInputs.forEach(input => {
      input.addEventListener("input", () => {
        ...
        if (value === "") {
          recalcLine(tr);
          if (container) updateSaleItemSummary(container);
    ...
    btnSave.addEventListener("click", async () => {
      ...
      const inputQuantityValue = Number(inputQuantity.value);
      ...
      try {
        ...
        if (!produto) {
          await showMessage("ID do produto é inválido.");
          return;
        }
...
export async function collectSaleItems(container: HTMLElement) {
